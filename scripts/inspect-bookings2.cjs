...
(async () => {
  ...
  if (!Array.isArray(resp)) {
    console.log('Unexpected response:', JSON.stringify(resp, null, 2).substring(0, 500));
    return;
  }
  resp.filter(d => d.document).forEach(d => {
    const f = d.document.fields;
    const id = d.document.name.split('/').pop();
    const status = (f.status && f.status.stringValue) || '(no status)';
    const driverId = (f.driverId && f.driverId.stringValue) || '(no driver)';
    const renter = (f.renterId && f.renterId.stringValue || '').slice(0, 12);
    const pickup = (f.pickupAddress && f.pickupAddress.stringValue || '').slice(0, 30);
