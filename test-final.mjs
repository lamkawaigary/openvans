...
await page.evaluate(() => {
  ...
  const suggs = allDivs.filter(d => d.textContent === '干諾道中');
...
console.log('=== All OpenVans logs ===');
logs.filter(l => l.includes('OpenVans') || l.includes('select') || l.includes('coord') || l.includes('getPlace')).forEach(l => console.log(l));
